...
To facilitate research on this challenge, a series of TREC LiveQA evaluation campaigns\footnote{http://www.trec-liveqa.org} was introduced in 2015, where automated systems attempt to answer real user questions within a 1 minute period.
...
Each input question consisted of the short question title, body and category (Figure~\ref{fig:ya_question}).
A QA system had to provide an answer of 1000 characters or less within a 1 minute period.
...
In addition to the candidate answers themselves, CRQA extract certain meta-data, that helps to estimate the relevance of a passage to the current question.
For example, for regular web page paragraphs, it is useful to know the topic of the page (\eg from its title) and the context (such as text that immediately precedes it in the document).
...
In a fully automated setup the top ranked candidate is returned as the final answer to the question.

\subsection{Crowdsourcing module}
\label{sec:system:crowd}

Unfortunately, fully automated QA systems still struggle with many difficult questions~\cite{overviewliveqa15}, therefore we decided to explore crowdsourcing as one of the ways to help the system to deal with these questions.
...
Since in TREC LiveQA systems had only 60 seconds to answer each question, we start a timer when a question arrives, and the system waits to receive all worker contributions until the timer reaches 50 seconds to leave it some time to generate the final answer.
...
\begin{figure*}[h!t]
	\centering
	\includegraphics[width=0.8\textwidth]{img/crowd_ui}
...
\item Workers receive a list of answers and rate them until the timer expires. Answers, provided by the workers, are also rated, except for their authors. Each answer is rated on a scale from 1 to 4, using the official TREC LiveQA rating scale:
...
During the TREC LiveQA 2016 run we used a simple heuristic model, which ordered the answers by the average rating, and returned the top candidate, if its average score was $\geq 2.5$, otherwise the longest worker contributed answer.
After the challenge was over, we collected all the questions and answers, that were shown to the crowd workers.
The quality of each candidate answer was judged using traditional batch-model crowdsourcing on the above mentioned scale from 1 (bad) to 4 (excellent).
...
The rest of the questions from TREC LiveQA 2016 run was used to evaluate the quality of the model.
...
The experimental evaluation of our CRQA system was done on data from the official run of TREC LiveQA shared task, which happened on May 31, 2016.
...
\begin{itemize}
\item \textit{Automated QA}: fully automated QA system
\item \textit{CRQA}: automated QA system with the crowdsourcing module and trained re-ranking model
...
The introduction of the crowdsourcing module allowed CRQA to cover couple percents of the questions, for which the automated system was not able to generate any candidates, as well as select better candidates when it was possible using crowd ratings.
...
However, the absolute value of the average quality gain is different, \eg crowdsourcing had the biggest impact on the quality for the ``Travel'' category, which was one of the hardest for the fully automated system~\cite{savenkov_trecliveqa15}.
...
Such a strategy provides a uniform coverage for all the answers selected for rating, while the former puts more emphasis on the currently top scoring candidates.
...
The average score for answers generating when candidates are shuffled is \textit{2.508}, and it is \textit{2.539} when the candidates are sorted according to their model ranking score.
...
The next reasonable question is what is the cost of this improvements.
...
Importantly, CRQA significantly improved question quality and coverage compared to the starting automated-only system, and, surprisingly, was often able to return a better answer, compared to the traditional CQA system with millions of users (Yahoo! Answers) with answers collected more than \textit{two days} after the original posting time.
